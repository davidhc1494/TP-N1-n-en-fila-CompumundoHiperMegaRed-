...
void Tablero::verTablero(){
	...
	while(this->tablero->avanzarCursor()){
		...
		std::cout<<"    Plano N°: "<<numeroDePlano<<std::endl<<std::endl<<"      ";
...
bool Tablero::revisarEnZ(Casillero* casillero){
	...
	if(contadorDeFichasIguales == valorDeN){
		gano = true;
		std::cout<<"    HA GANADO EL JUGADOR Nº: "<<casillero->verFicha()->verNumeroDeJugador()<<" contador: "<<contadorDeFichasIguales<<std::endl;
	}
	return gano;
}
